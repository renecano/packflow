...
def resolve_agent(name: str | None) -> Agent:
    """Devuelve el agente pedido. 'greedy' siempre disponible; resto usa el activo."""
    if name == "greedy":
        return GreedyAgent()
    return agent
...
def pack(request: PackRequest) -> PackResponse:
    ...
    sol = solve(env, resolve_agent(request.agent))
...
async def stream(websocket: WebSocket) -> None:
    ...
    try:
        ...
        truck = (request.truck.W, request.truck.H, request.truck.D)
        env = make_env(request.boxes, truck, request.max_weight)
        for event in run_episode(env, resolve_agent(request.agent)):
